...
import {
  kelas, jadwal, tugas, materi, aktivitas,
...
import {
  ...
  unregisterKelas,
  infoKelas,
} from './handlers/kelasHandler.js';
import {
  tambahJadwal,
  hapusJadwal,
  getJadwal,
} from './handlers/jadwalHandler.js';
import { tambahTugas, hapusTugas, getTugas } from './handlers/tugasHandler.js';
import {
  uploadMateri,
  hapusMateri,
  getMateri,
} from './handlers/materiHandler.js';
import {
  updateAktivitas,
  getLeaderboard,
} from './handlers/gamifikasiHandler.js';
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    for (const m of messages) {
      try {
        ...
        console.log(
          `Received message: ${text} from ${sender} in ${groupId}`,
        );
...
async function processCommand(sock, m, groupId, sender, text) {
  ...
  if (text.startsWith('!register')) {
    ...
    if (!newKelasName) {
      await reply(
        'Format salah. Gunakan: !update_kelas <nama_kelas_baru>',
      );
      return;
    }
  ...
  // Handler untuk jadwal kelas
    ...
    if (!args) {
      await reply(
        'Format salah. Gunakan: !tambah_jadwal <hari> | <jam_awal>-<jam_akhir> | <mata_kuliah> | <ruang>',
      );
      return;
    }

    const [hari, jam, mataKuliah, ruang] = args
      .split('|')
      .map((arg) => arg.trim()); // Pisahkan berdasarkan '|'
    ...
    if (!jamAwal || !jamAkhir) {
      await reply(
        'Format jam salah. Gunakan <jam_awal>:<jam_akhir> (contoh: 08:00-09:00)',
      );
      return;
    }

    const response = await tambahJadwal(
      groupId,
      hari.toLowerCase(),
      `${jamAwal}-${jamAkhir}`,
      mataKuliah,
    ...
    if (!args) {
      await reply(
        'Format salah. Gunakan: !hapus_jadwal <hari> | <mata_kuliah>',
      );
      return;
    }

    const [hari, mataKuliah] = args.split('|').map((arg) => arg.trim()); // Pisahkan berdasarkan '|'
    const response = await hapusJadwal(
      groupId,
      hari.toLowerCase(),
      mataKuliah,
    );
  ...
  // Handler untuk tugas
    ...
    if (!args) {
      await reply(
        'Format salah. Gunakan: !tambah_tugas <mata_kuliah> | <judul> | <deadline>',
      );
      return;
    }

    const [mataKuliah, judul, deadline] = args
      .split('|')
      .map((arg) => arg.trim()); // Pisahkan berdasarkan '|'
    ...
    if (isNaN(deadlineDate.getTime())) {
      await reply(
        'Format deadline salah. Gunakan format YYYY-MM-DD HH:mm (contoh: 2022-12-31 23:59)',
      );
      return;
    }

    const response = await tambahTugas(
      groupId,
      mataKuliah,
      judul,
      deadline,
    ...
    if (!args) {
      await reply(
        'Format salah. Gunakan: !hapus_tugas <mata_kuliah> | <judul>',
      );
      return;
    }
  ...
  // Handler untuk materi
    ...
    if (!args) {
      await reply(
        'Format salah. Gunakan: !upload_materi <mata_kuliah> | <judul> | <link>',
      );
      return;
    }

    const [mataKuliah, judul, link] = args
      .split('|')
      .map((arg) => arg.trim()); // Pisahkan berdasarkan '|'
    ...
    if (!args) {
      await reply(
        'Format salah. Gunakan: !hapus_materi <mata_kuliah> | <judul>',
      );
      return;
    }
  ...
  // Handler untuk gamifikasi
    ...
    if (!resp.url) {
      return await reply(
        'Maaf, tidak dapat mengambil meme saat ini. Coba lagi nanti.',
      );
    }
...
`;
    ...
    await reply(
      'Mode silent diaktifkan. Bot tidak akan mengirim notifikasi otomatis.',
    );
  } else if (text.startsWith('!mode_aktif')) {
    // Implementasi mode aktif (aktifkan notifikasi otomatis)
    await reply(
      'Mode aktif diaktifkan. Bot akan mengirim notifikasi otomatis.',
    );
  }
}
...
function setupCronJobs(sock) {
      ...
      if (jadwal.data[groupId] && jadwal.data[groupId][hariIni]) {
        ...
        if (jadwalToday.length > 0) {
          const jadwalText = jadwalToday
            .map(
              (j) => `🕒 *${j.jam}* - 📚 *${j.mataKuliah}*${j.ruang ? ` (📍 *${j.ruang}*)` : ''}`,
            )
            .join('\n');
  ...
  //                 console.log("selisihMenit", selisihMenit);
...
// function untuk send whatsapp message
...
export { startBot, sendWhatsappMessage};
